 ...
 * 3. Archives the old setup to a timestamped backup, then removes it so craft
 *    can generate a fresh configuration (the backup lets the user revert)
 *
...
import { existsSync, readFileSync, writeFileSync, readdirSync, rmSync, statSync, unlinkSync, mkdirSync } from 'fs';
...
/**
 * Resolve the set of paths that removeExistingSetup() will delete and that
 * therefore need backing up. Only the generated USER_GUIDE.md is included.
 *
 * @param {string} targetDir - Project root
 * @returns {Array<{ path: string, name: string, dir: boolean }>}
 */
function collectRemovableItems(targetDir) {
  const items = [];

  const claudeDir = join(targetDir, '.claude');
  if (existsSync(claudeDir)) items.push({ path: claudeDir, name: '.claude', dir: true });

  const claudeMdPath = join(targetDir, 'CLAUDE.md');
  if (existsSync(claudeMdPath)) items.push({ path: claudeMdPath, name: 'CLAUDE.md', dir: false });

  const userGuidePath = join(targetDir, 'USER_GUIDE.md');
  if (existsSync(userGuidePath)) {
    try {
      if (readFileSync(userGuidePath, 'utf8').includes('Generated by claude-craft')) {
        items.push({ path: userGuidePath, name: 'USER_GUIDE.md', dir: false });
      }
    } catch { /* ignore unreadable guide */ }
  }
...
/**
 * Archive the existing setup (the .claude/ directory plus the generated
 * CLAUDE.md / USER_GUIDE.md) into a timestamped backup under
 * `<project>/.claude-backup/` so the user can revert after a reinstall.
 *
 * Produces a single `.zip` when possible; if zipping is unavailable for any
 * reason (e.g. the optional `adm-zip` dependency is missing), it falls back to
 * a copied backup directory so a recoverable copy always exists before anything
 * is deleted. A `.gitignore` is written into the backup folder so the previous
 * config — which may contain settings/secrets — is never committed by accident.
 *
 * @param {string} targetDir - Project root
 * @returns {Promise<string|null>} Absolute path to the backup, or null if there
 *   was nothing to back up / the backup could not be created.
 */
export async function backupExistingSetup(targetDir) {
  const items = collectRemovableItems(targetDir);
  if (items.length === 0) return null;

  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const backupRoot = join(targetDir, '.claude-backup');

  const ensureBackupRoot = () => {
    mkdirSync(backupRoot, { recursive: true });
    // Keep backups (and any secrets in the old config) out of version control.
    try { writeFileSync(join(backupRoot, '.gitignore'), '*\n'); } catch { /* best effort */ }
  };

  // 1) Preferred: a single .zip archive.
  try {
    const AdmZip = (await import('adm-zip')).default;
    ensureBackupRoot();
    const zipPath = join(backupRoot, `config-${stamp}.zip`);
    const zip = new AdmZip();
    for (const item of items) {
      if (item.dir) zip.addLocalFolder(item.path, item.name);
      else zip.addLocalFile(item.path);
    }
    zip.writeZip(zipPath);
    return zipPath;
  } catch (err) {
    logger.debug(`Zip backup unavailable (${err.message}); falling back to a copied backup directory.`);
  }

  // 2) Fallback: a copied backup directory (always available via fs-extra).
  try {
    ensureBackupRoot();
    const backupDir = join(backupRoot, `config-${stamp}`);
    fse.ensureDirSync(backupDir);
    for (const item of items) {
      fse.copySync(item.path, join(backupDir, item.name));
    }
    return backupDir;
  } catch (err) {
    logger.warn(`Could not back up the previous configuration: ${err.message}`);
    return null;
  }
}

/**
 * Back up, then remove, the existing .claude/ directory and the generated root
 * CLAUDE.md / USER_GUIDE.md. The previous configuration is archived first (see
 * backupExistingSetup) so the user can always revert. Leaves .gitignore and
 * .plans/ untouched (security patterns and plans should persist).
 *
 * @param {string} targetDir - Project root
 * @returns {Promise<{ backupPath: string|null }>}
 */
export async function removeExistingSetup(targetDir) {
  let backupPath = null;
  try {
    backupPath = await backupExistingSetup(targetDir);
  } catch (err) {
    logger.warn(`Failed to back up previous configuration: ${err.message}`);
  }
